...
if CODEDISPERSE:
    ...
    for i, freq in enumerate(reversed_frequencies):
        ...
        padded_re[:, idx1:idx2] = data[i, 0:number_segments * smearing_samples[i], 0].reshape(
            [number_segments, smearing_samples[i]])
        padded_im[:, idx1:idx2] = data[i, 0:number_segments * smearing_samples[i], 1].reshape(
            [number_segments, smearing_samples[i]])
        ...
        intra_frequencies = np.arange(-freq_resolution / 2, freq_resolution / 2, freq_resolution / fft_size) + freq
        ...
        data[i, 0:number_segments * smearing_samples[i], 0] = codep_t[:, idx1:idx2].reshape(
            number_segments * smearing_samples[i]).real
        data[i, 0:number_segments * smearing_samples[i], 1] = codep_t[:, idx1:idx2].reshape(
            number_segments * smearing_samples[i]).imag
...
if MEAS_RESIDUAL_DISP:
    ...
    for i, meas_ch in enumerate(meas_chs):
        ...
        for j in np.arange(new_num_pulses):
            ...
            magnitude = np.abs(
                fine_pfb[0:int(fine_fft_len / 2), start:end] * np.conj(fine_pfb[0:int(fine_fft_len / 2), start:end]))
            redisp_summed_profile[int(i * fine_fft_len / 2):int((i + 1) * fine_fft_len / 2),
            :] += magnitude.real  # re**2 + im**2
        ...
        if i == 0:
            ...
            delay = dispersion_constant * vela_dm * (1 / (freq ** 2) - 1 / (f2 ** 2))  # ms
        ...
        redisp_summed_profile[int(i * fine_fft_len / 2):int((i + 1) * fine_fft_len / 2), :] = np.roll(
            redisp_summed_profile[int(i * fine_fft_len / 2):int((i + 1) * fine_fft_len / 2), :], -num_2_roll)
        # print(redisp_summed_profile[int(i*fine_fft_len/2):int(i*fine_fft_len/2)+5, 0:5])

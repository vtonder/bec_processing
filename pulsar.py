...
class Pulsar():
    ...
    def __init__(self, period, dm, file_name, fp):
        ...
        self.folded_profile = np.zeros([no_channels, fp * self.samples_int_T])
        self.inverted_folded = np.zeros([no_channels, fp * self.samples_int_T])
    ...
    def incoherent_dedisperse(self):
        ...
        for i, freq in enumerate(frequencies):
            ...
            delay = c*self.dm*(1/(f2**2) - 1/(freq**2))
    ...
    def frequency_phase_power_spectrum(self):
        ...
        for i in np.arange(no_channels):
            ...
            self.inverted_folded[(no_channels - 1) - i, :] = self.folded_profile[i, :] - mean

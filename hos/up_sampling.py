...
pulse_train_up2 = pulse_train_up[0:N]
b = Bispectrum(pulse_train, reshape=True, fft_size=fft_size, method='direct')
b_up = Bispectrum(pulse_train_up2, reshape=True, fft_size=fft_size, method='direct')
...
plt.plot(k, bup_ps/5, label=r'$\uparrow$ pulse train')
plt.xlim(-1024,1024)
plt.ylabel("PSD of line code")
plt.xlabel("frequency samples $k$")
plt.legend(loc=7)
plt.grid()
#plt.savefig('/home/vereese/Documents/PhD/Thesis/Figures/psd_pt.pdf', transparent=True, bbox_inches='tight')
#plt.savefig('/home/vereese/thesis_pics/psd_pt.pdf', transparent=True, bbox_inches='tight')

plt.figure(2)
b1 = np.abs(b.full_bispec)
maxi = np.max(b1)
mini = np.min(b1)
plt.imshow(b1, aspect='auto', origin='lower', vmin = mini, vmax = maxi/3, extent=([-1024, 1024, -1024, 1024]))
plt.xlabel("frequency samples $k$")

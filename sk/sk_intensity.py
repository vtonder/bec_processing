...
if rank > 0:
    ...
    comm.Send([skx_flags, MPI.DOUBLE], dest=0, tag=16)
    comm.Send([sky_flags, MPI.DOUBLE], dest=0, tag=17)
    comm.Send([summed_flags, MPI.DOUBLE], dest=0, tag=18)
    comm.Send([num_nz, MPI.DOUBLE], dest=0, tag=19)
else:
    for i in range(1, size):
        ...
        comm.Recv([tmp_summed_flags, MPI.DOUBLE], source=i, tag=18)
        comm.Recv([tmp_num_nz, MPI.DOUBLE], source=i, tag=19)

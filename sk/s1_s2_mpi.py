...
for i, ld_idx in enumerate(np.arange(start, stop, time_chunk_size)):
    ...
    for j, idx in enumerate(np.arange(0, time_chunk_size, M)):
        s1[:, s_idx_offset + j], s2[:, s_idx_offset + j] = s1_s2(local_data[:, idx:idx + M, 0] +
                                                                 1j*local_data[:, idx:idx + M, 1], FFT_LEN * 2)
...
if rank > 0:
    ...
    np.save('S1_M' + str(M) + tag + pol, tot_S1)
    np.save('S2_M' + str(M) + tag + pol, tot_S2)

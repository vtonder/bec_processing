...
# to get pulse start and stop read off FWHM from intensity profile: ((4.9270-3.97325)/2) + 3.97325 = 4.450125 [units in 10**10] 
pulse_start = 3035 
pulse_stop = 3162
...
for i, pulse_i in enumerate(np.arange(rank * np_rank, (rank + 1) * np_rank)):
    ...
    profile = np.float32(incoherent_dedisperse(sp_x + sp_y, tag)).sum(axis=0)
    snr = 0
    m = np.mean(profile[0:pulse_w])
    s = np.std(profile[0:pulse_w])
    for j in np.arange(pulse_w):
        snr = snr + (profile[pulse_start + j] - m)
...
if rank > 0:
    ...
    tot_snrs = np.zeros(np_rank*size)
    tot_snrs[0:np_rank] = snrs
    for i in range(1, size):
        tmp_snrs = np.zeros(np_rank)
        comm.Recv([tmp_snrs, MPI.DOUBLE], source=i, tag=15)
        tot_snrs[i*np_rank:i*np_rank + np_rank] = tmp_snrs

    np.save('snrs', tot_snrs)
